...
def create_rag_generator(llm: str | None = None) -> LLMResponseGenerator:
    choice = (llm or "auto").strip().lower()

    use_claude = choice == "claude" or (choice == "auto" and settings.claude_token)
    if use_claude and settings.claude_token:
        llm_adapter = ClaudeAdapter(model=settings.claude_model, token=settings.claude_token)
        ...
        llm_adapter = HuggingFaceAdapter(model=settings.hf_llm_model, token=settings.hf_token)
        max_tokens = settings.hf_llm_max_tokens
        logging.info("Using HuggingFace adapter (model: %s)", settings.hf_llm_model)
        resolved = "hf"

    generator = LLMResponseGenerator(
        llm=llm_adapter, max_tokens=max_tokens, temperature=settings.llm_temperature
    )
    generator.resolved_llm = resolved  # type: ignore[attr-defined]
    return generator
